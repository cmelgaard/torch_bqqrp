...
int BQRRP_GPU<T, RNG>::call(
    ...
    for(iter = 0; iter < maxiter; ++iter) {
		...
		//printf("ALLOCATING d_work_geqrf_opt, iter=%d\n", iter);  // TODO: DEBUG
        ...
        if (block_rank != b_sz_const) {
            if (iter == 0) {
                ...
                cusolverDnSormqr_bufferSize(cusolverH, CUBLAS_SIDE_LEFT, CUBLAS_OP_T, block_rank, cols - b_sz, block_rank, A_work, lda, &tau[iter * b_sz], Work1, lda, &lwork_ormqr);
                // Allocate workspace
                cudaMalloc(reinterpret_cast<void **>(&d_work_ormqr), sizeof(double) * lwork_ormqr);
            }
            cusolverDnSormqr(cusolverH, CUBLAS_SIDE_LEFT, CUBLAS_OP_T, block_rank, cols - b_sz, block_rank, A_work, lda, &tau[iter * b_sz], Work1, lda, d_work_ormqr, lwork_ormqr, d_info_cusolver);
            // Synchronization required after using cusolver
            cudaStreamSynchronize(strm);
        } else {
            if (iter == 0) {
                // Compute optimal workspace size
                cusolverDnSormqr_bufferSize(cusolverH, CUBLAS_SIDE_LEFT, CUBLAS_OP_T, rows, cols - b_sz, block_rank, A_work, lda, &tau[iter * b_sz], Work1, lda, &lwork_ormqr);
                // Allocate workspace
                cudaMalloc(reinterpret_cast<void **>(&d_work_ormqr), sizeof(double) * lwork_ormqr);
            }
            cusolverDnSormqr(cusolverH, CUBLAS_SIDE_LEFT, CUBLAS_OP_T, rows, cols - b_sz, block_rank, A_work, lda, &tau[iter * b_sz], Work1, lda, d_work_ormqr, lwork_ormqr, d_info_cusolver);
            // Synchronization required after using cusolver
            cudaStreamSynchronize(strm);
        }
	    ...
	    cusolverDnDestroy(cusolverH);
	    //printf("RETURN LOCATION 2, iter=%d\n", iter); TODO: DEBUG
            return 0;
        }
        if(this -> timing) {
