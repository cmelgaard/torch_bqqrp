...
@torch.compile
def lisao_via_QR(G, steps=3, eps=1e-7):
    """
    ...
    A_factored, tau = torch.geqrf(mat)
    ...
    def step(self):
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                update = lisao_via_QR(g.reshape(len(g), -1)).view(g.shape) # whiten the update
...
def main(run, model):
    ...
    optimizer2 = Lisao(filter_params, lr=0.24, momentum=0.6, nesterov=True)
